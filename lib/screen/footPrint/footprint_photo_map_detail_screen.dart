...
class _FootprintPhotoMapDetailScreenState
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => FootprintPhotoMapOverlayProvider(MapType.KOREA_FULL.type),
              ...
              floatingActionButton: FloatingActionButton(
                ...
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const FootprintHistoryEditScreen()));
                },
              ),
              body: NaverMap(
                onMapReady: (NaverMapController controller) {
                  _mapController = controller;
                  _addOverlay(provider);
                },
  ...
  Future<void> _addOverlay(FootprintPhotoMapOverlayProvider provider) async {
  ...
  Future<void> _loadmapdata(FootprintPhotoMapOverlayProvider provider) async {
    ...
    for (final feature in geojson.features) {
      ...
      final properties = feature.properties; // AREA_CD, AREA_ENG_NM, AREA_KOR_NM
      ...
      for (final polygon in geoJsonMultiPolygon.polygons) {
        ...
        var overlay = NPolygonOverlay(
            ...
            outlineColor: ColorFamily.black,
            outlineWidth: 1
        );
        // 해당 구역을 클릭 했을 때 리스너
        overlay.setOnTapListener((overlay){
          print("enung ${overlay.info.id}");
          print("enung ${provider.polygonInfos.length}");
        });
        provider.addInfo(_polygonPoints.length, [_ENG_NM, _KOR_NM]);
